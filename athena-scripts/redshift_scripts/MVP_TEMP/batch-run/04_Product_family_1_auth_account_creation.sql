...
FROM (
  SELECT *
  FROM dev_conformed.v_stg_auth_account_creation
  WHERE EVENT_NAME IN (
    SELECT EVENT_NAME
...
SELECT DISTINCT CASE 
    ...
    CURRENT_DATE,
    9999
FROM dev_conformed.v_stg_auth_account_creation AS st
WHERE (CASE 
        WHEN st.EVENT_NAME LIKE '%IPV%' THEN 'Web'
        WHEN st.EVENT_NAME LIKE '%DCMAW%' THEN 'App'
        ELSE 'General'
...
FROM (
        ...
        CURRENT_DATE,
        9999
        FROM dev_conformed.v_stg_auth_account_creation mn
