...
FROM (
  SELECT *
  FROM dev_conformed.v_stg_ipv_cri_driving_license
  WHERE EVENT_NAME IN (
    SELECT EVENT_NAME
...
SELECT DISTINCT CASE 
    ...
    CURRENT_DATE,
    9999
FROM dev_conformed.v_stg_ipv_cri_driving_license AS st
WHERE (CASE 
        WHEN st.EVENT_NAME LIKE '%IPV%' THEN 'Web'
        WHEN st.EVENT_NAME LIKE '%DCMAW%' THEN 'App'
        ELSE 'General'
...
FROM (
        ...
        CURRENT_DATE,
        9999
        FROM dev_conformed.v_stg_ipv_cri_driving_license mn
...
UPDATE dev_conformed.BatchControl 
...
WHERE dev_conformed.BatchControl.Product_family =subquery.PRODUCT_FAMILY;
